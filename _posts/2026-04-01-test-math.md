...
Cho một tập hợp là các số tự nhiên \\\(\mathbb{N}\\\):
...
Tập hợp năm số tự nhiên \\\(\mathbb{N}\\\) đầu tiên được biểu diễn ở dạng liệt kê như sau:
...
Tập hợp các số hữu tỉ \\\(\mathbb{Q}\\\) được biểu diễn bằng cách chỉ rõ tính chất đặc trưng của mỗi phần tử như sau: 
...
**Định nghĩa 2.1**: Cho *X* và *Y* là hai tập hợp khác rỗng. Một ánh xạ $f$ từ $X$ đến $Y$ là một quy tắc cho tương ứng mỗi phần tử $x \in X$ với một và chỉ một phần tử $y \in Y$. Phần tử *x* được gọi là tạo ảnh của phần tử *y* qua ánh xạ *f*. Phần tử *y* được gọi là ảnh của phần tử *x* qua ánh xạ *f* và kí hiệu là *f(x)*.
...
Ánh xạ $ f: \mathbb{R} \to \mathbb{R}, x \mapsto x^2 $. Vì $f(2) = f(-2) = 2^2 = (-2)^2 = 4$ nên 4 là ảnh của 2 và -2. Còn 2 và -2 đều là tạo ảnh của 4.
