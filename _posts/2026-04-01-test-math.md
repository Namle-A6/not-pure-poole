...
> Giả sử \\\(A = (a_{ij})_{n \times n}\\\) là một ma trận vuông cấp \\\(n\\\) với các phần tử là số thực hoặc số phức. Định thức của ma trận \\\(A\\\), ký hiệu là \\\(\det(A)\\\) hay |\\\(A\\\)|được cho bởi:
